...
mocap_motions = [
  # ["pace", "data/dog_walk00_joint_pos.txt",0,-1],
  ["trot", "data/dog_walk03_joint_pos.txt",100,481 ],
  ...
  # ["right turn0", "data/dog_walk09_joint_pos.txt", 1700,2450],
]

# mocap_motions = [[f'motion_{i}', file, 0,-1] for i, file in enumerate(files) ]
...
def set_pose(robot, pose):
  ...
  for j in range(num_joints):
    ...
    j_pose_idx = j_info[3]
    print(j_pose_idx, j_info[12].decode("utf-8") )
...
def get_frame_local_roations(robot, p, log=False):
  ...
  for joint_name in joints:
      ...
      if joint_index != -1:  # Check if the joint name exists in the model
          ...
          # print(joint_position, joint_name)
          ...
          print(joint_name, joint_info[13])
          input()
  ...
  if log:
    for joint_name, joint_angle in joint_angles.items():
        print(f"Joint '{joint_name}' Angle: {joint_angle} radians")
...
def main(argv):
  ...
  while True:
    ...
    for mocap_motion in mocap_motions:
      ...
      print(config.DEFAULT_JOINT_POSE)
      set_pose(robot, np.concatenate([config.INIT_POS, config.INIT_ROT, config.DEFAULT_JOINT_POSE]))

      print(list_all_joint_names(robot, p))
      print(len(list_all_joint_names(robot, p)))
      print_all_body_names(robot, p)
      input()
      ...
      store_rots = np.array(store_rots)
      fr = store_rots[:, [1,2,3,4]]
      fl = store_rots[:, [5,6,7,8]]
      rr = store_rots[:, [9,10,11,12]]
      rl = store_rots[:, [13,14,15,16]]
      base = store_rots[:, [0]]

      store_rots = np.concatenate((base,fl,fr,rl,rr), axis=1)
      store_pos = np.array(store_pos)
      print(store_rots.shape)
      print(store_pos.shape)
      np.save('./rotations-v6.npy', store_rots)
      np.save('./positions-v6.npy', store_pos)
      quit()
